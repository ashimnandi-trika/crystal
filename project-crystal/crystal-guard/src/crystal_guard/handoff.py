...
def generate_handoff_prompt(
    ...
    # Section 6: Auto Session Summary
    if baseline_changes:
        added_files = next((c for c in baseline_changes if c["metric"] == "files" and c["delta"] > 0), None)
        fixed_issues = next((c for c in baseline_changes if c["metric"] == "violations" and c["delta"] < 0), None)
        summary_parts = []
        if added_files:
        ...
        if summary_parts:
            lines.append("## THIS SESSION SUMMARY")
            lines.append(f"This session you: {', '.join(summary_parts)}.")
            lines.append("")

    # Section 7: Test Regression Detection
    if baseline_changes:
        test_regression = next((c for c in baseline_changes if c["metric"] == "test files" and c["is_regression"]), None)
        if test_regression:
            lines.append("## TEST REGRESSION WARNING")
            lines.append(f"Test files went from {test_regression['previous']} to {test_regression['current']}. Tests may have been deleted.")
            lines.append("")

    # Section 8: PRD Context
    if prd_content:
    ...
    # Section 9: Next Session Priorities
    lines.append("## NEXT SESSION PRIORITIES")
    if health_report.critical_count > 0:
        lines.append(f"1. Fix {health_report.critical_count} CRITICAL issue(s) immediately")
    if health_report.high_count > 0:
        lines.append(f"{'2' if health_report.critical_count > 0 else '1'}. Fix {health_report.high_count} HIGH severity issue(s)")
    if health_report.critical_count == 0 and health_report.high_count == 0:
        lines.append("1. No blocking issues. Continue building new features.")
    if health_report.medium_count > 0:
        lines.append(f"- Clean up {health_report.medium_count} medium issues when convenient")
    lines.append("")

    # Section 10: Instructions for the AI
    ...
    lines.append("")
    lines.append("---")
    lines.append("*Generated by Crystal v0.3.0 — https://crystalcodes.dev*")
